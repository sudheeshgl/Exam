...
if __name__ == '__main__':
    ...
    while True:
        print('*' * 50)
        print('             Competitive Examination')
        print('*' * 50)
        print('Please add the details is Config.txt File')
        a=input('Are you sure (y or n)')
        if a=='y':
            with open(config.get('exam','answersheet'), 'r') as sheet, open(config.get('exam','answer'), 'r') as answer:
                ...
                # print('''
                # 1.Display Total Students with Rank
                # 2.Display By Catagorie
                # ''')
                # options = input('Enter the Options :')
                # if options == '1':
                # totalrank=listcategories[0]
                # for i in range(0,len(totalrank)):
                #     t = PrettyTable(['No','Register No','Name','Mark','Negative Mark','Total','Rank'])
                #     t.add_row([i+1])
                #     print(t)
        else:
            print('Bye')
